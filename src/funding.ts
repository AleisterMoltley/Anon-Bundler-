...
export async function fundWallets(connection: Connection, master: Keypair, wallets: Keypair[]) {
  ...
  const txFeeBudget = 50_000 * Math.ceil(wallets.length / 8);
  ...
  log.info(
    `Required: ~${(required / LAMPORTS_PER_SOL).toFixed(4)} SOL (${wallets.length} wallets × ${CONFIG.solPerWallet} SOL + fees)`
  );
